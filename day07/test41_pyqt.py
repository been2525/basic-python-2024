...
class WinApp(QWidget):
    ...
    def initUI(self):
        # 이미지 추가 .scaledToWidth(800) 큰 해상도를 W800으록 고정
        ...
        lblSize.setStyleSheet('Color: purple;')
        lblSize.setText(f'{pixmap.width()} x {pixmap.height()}') # kitty.jpg의 width x height
        lblSize.setAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter) # 가로중앙정렬 | 세로중앙정렬
        ...
        self.setGeometry(rect) # 같은 이름의 함수를 여러개 선언해놓고 입맛에 맞게 골라쓰는
...
if __name__ == '__main__':
    ...
    screen_rect = app.desktop().screenGeometry() # 모니터 해상도구하는 법1
